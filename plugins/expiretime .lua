...
local function pre_process(msg)
 msg.text = msg.content_.text
	...
	if tonumber(timetoexpire) == 0 then
		...
		tdcli.sendMessage(msg.chat_id_, 0, 1, '🔴 Only *0* Days,For Buy @Mr_TelgramBot', 1, 'md')
		redis:hset('expires0',msg.chat_id_,'5')
	end
	if tonumber(timetoexpire) == 1 then
		if redis:hget('expires1',msg.chat_id_) then end
		tdcli.sendMessage(msg.chat_id_, 0, 1, '🔴 Only *1* Days,For Buy @Mr_TelgramBot', 1, 'md')
		redis:hset('expires1',msg.chat_id_,'5')
	end
	if tonumber(timetoexpire) == 2 then
		if redis:hget('expires2',msg.chat_id_) then end
		tdcli.sendMessage(msg.chat_id_, 0, 1, '🔴 Only *2* Days,For Buy @Mr_TelgramBot', 1, 'md')
		redis:hset('expires2',msg.chat_id_,'5')
	end
	if tonumber(timetoexpire) == 3 then
		if redis:hget('expires3',msg.chat_id_) then end
		tdcli.sendMessage(msg.chat_id_, 0, 1, '🔴 Only *3* Days,For Buy @Mr_TelgramBot', 1, 'md')
		redis:hset('expires3',msg.chat_id_,'5')
	end
	if tonumber(timetoexpire) == 4 then
		if redis:hget('expires4',msg.chat_id_) then end
		tdcli.sendMessage(msg.chat_id_, 0, 1, '🔴 Only *4* Days,For Buy @Mr_TelgramBot', 1, 'md')
		redis:hset('expires4',msg.chat_id_,'5')
	end
	if tonumber(timetoexpire) == 5 then
		if redis:hget('expires5',msg.chat_id_) then end
		tdcli.sendMessage(msg.chat_id_, 0, 1, '🔴 Only *5* Days,For Buy @Mr_TelgramBot', 1, 'md')
...
function run(msg, matches)
	if matches[1]:lower() == 'setexpire' then
		...
		return "✅ *Expire Time Set* _"..matches[2].. "_ *Days*,Tnx  🙏🏻"
	end
